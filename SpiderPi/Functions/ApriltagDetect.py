...
def initMove():
    ...
    AK.setPitchRangeMoving((0, 12, 35), 30, -90, 100, 1000)
    ...
    while True:
        ...
        if __isRunning:
            if tag_id is not None:
                ...
                time.sleep(0.5)
                if tag_id == 1:
                    AGC.runActionGroup('wave',lock_servos=LOCK_SERVOS)#招手
                    tag_id = None
                    time.sleep(1)                  
                    action_finish = True                
                elif tag_id == 2:                    
                    AGC.runActionGroup('stepping',lock_servos=LOCK_SERVOS)#原地踏步
                    tag_id = None
                    time.sleep(1)
                    action_finish = True          
                elif tag_id == 3:                   
                    AGC.runActionGroup('twist_l',lock_servos=LOCK_SERVOS)#扭腰
                    tag_id = None
                    time.sleep(1)
                    action_finish = True
                else:
