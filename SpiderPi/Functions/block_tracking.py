...
centre_x = 320
centre_y = 240
x_pid = PID(P=0.08, I=0.005, D=0.008)#pid初始化
    ...
    while True:
        if world_x > 0 or world_y > 0:
            ...
            if abs(world_x-centre_x) >= 10:
                x_pid.SetPoint = centre_x  #设定
            else:
                x_pid.SetPoint = world_x
            x_pid.update(world_x)  #当前
            dx = int(x_pid.output)/10 # 输出
            x -= dx
            
            # Y轴PID处理
            if abs(world_y-centre_y) >= 10:
                y_pid.SetPoint = centre_y  #设定
            else:
                y_pid.SetPoint = world_y
            y_pid.update(world_y)  #当前
            dy = int(y_pid.output)/10 # 输出
            y += dy
            if x > 10 : x = 10
            if x < -10 : x = -10
            if y > 24 : y = 24
            if y < 6 : y = 6
            AK.setPitchRangeMoving((x, y, 5), -90, -90, 100, 80) # 移动到目标位置
            time.sleep(0.05)
            world_x, world_y = -1, -1
